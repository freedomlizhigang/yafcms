...
class Model {
	...
	public function __construct() {
		...
		$this->db = Db_Db::get_instance($this->db_config)->get_database($this->db_setting);
	}
	...
	final public function listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array(), $data = '*') {
		...
		$this->pages = $this->pages($this->number, $page, $pagesize, $array, $setpages);
		$array = array();
		if ($this->number > 0) {
	 ...
	 * @return boolean
	 */
	final public function insert_all($data, $return_insert_id = false, $replace = false) {
		return $this->db->insert_all($data, $this->table_name, $return_insert_id, $replace);
	}
